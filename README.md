...
- 某一路打开后 5 秒仍无帧回调时会自动重开，最多重试 2 次；重试会保持 MJPEG 优先，只有在 320x240 或更低分辨率时才进一步降低带宽系数，用来验证是否为 USB 带宽/调度压力。
- 启动日志会显示当前 App 版本；每路打开后会单独打印“自动重试监控”启动和 5 秒检查结果，避免现场看不出为什么没有触发重试。
- 当前启用强低带宽诊断模式：打开摄像头时优先选择 320x240 或更低的 MJPEG 分辨率，预览 FPS 先请求 1-10；如果摄像头不支持该 FPS 档，会自动回退到 1-31。选到 320x240 或更低时带宽系数从 0.20 起步，重试时降到 0.10；如果摄像头只能退到 640x480 这类更高分辨率，会改用 1.00 兼容带宽系数，避免高分辨率被低系数压到无帧。
- 多路摄像头不会瞬间连续打开，而是按约 900ms 错峰请求 USB 授权，降低多路 startPreview 同时争抢 USB 调度资源的概率。
- 如果摄像头不支持 320x240 或更低分辨率，会退到它支持的最小分辨率并在日志里提示，同时改用兼容带宽系数，方便判断该路是否仍然有较高带宽压力。
...
- 只能打开一路：可能是 USB 带宽、电源、摄像头格式、机器人内核/驱动限制；当前 App 已启用强低带宽和错峰打开，仍不稳定时更应优先检查 Hub、电源、USB 口拓扑或摄像头支持列表。
...
- 日志里出现“低FPS不兼容”或 `fpsFallback=true`：说明摄像头拒绝 1-10 FPS 参数，App 已自动回退到 1-31 FPS 兼容档继续打开。
- 日志里出现“自动重试”：说明某一路打开后持续无帧，App 已主动重开；如果低分辨率强低带宽重开后恢复，偏向 USB 带宽/调度或首次打开时序问题；如果 640x480 兼容带宽下两次后仍无帧，优先换 Hub 口、线材或单独测试该摄像头。
