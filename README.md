...
- 某一路打开后 5 秒仍无帧回调时会自动重开，最多重试 2 次；兼容恢复模式下会先改试其它 MJPEG 分辨率，再改试 YUYV。YUYV 兜底不绑定预览窗口，但会尝试转成 JPEG 用于拉流/截图，避免绿屏和闪退。
...
- 日志里出现“YUYV转JPEG模式”或“来源=YUYV->NV21”：说明这一路 YUYV 能收到帧，并且 App 正在尝试生成 JPEG 给拉流/截图使用。
