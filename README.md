...
- 顶部按钮区提供“扫描/打开”、“关闭全部”、“只开第1路”、“显示日志/隐藏日志”，日志面板展开后显示在预览区上方。
...
- 需要验证第 1 路是否被多路并发影响时，点顶部“只开第1路”；App 会关闭其它路，只打开扫描到的第 1 个 UVC 设备，并继续验证 `/stream/0.mjpeg`。
- 日志面板里第 1 路相关日志会用黄色加粗显示，现场拍照时优先拍这些高亮行，方便确认第 1 路是否出现 `格式=RAW`、`帧回调首次到达`、`buffer=614400` 或 `JPEG已生成`。
- 日志面板手动往上翻查看历史日志时，新日志不会再强制拉回底部；现场可停在关键黄色行拍照，滚到底部或重新展开日志后再继续看最新日志。
- 日志里出现“Surface抓图JPEG已生成”或“来源=TextureView->JPEG”：说明第 1 路即使没有 Java `frameCallback`，也已经从真实预览 Surface 抓到了 JPEG，现场应立即测试 `/stream/0.mjpeg` 和 `/snapshot/0.jpg`。
