...
- 某一路打开后 5 秒仍无帧回调时会自动重开，最多重试 2 次；兼容恢复模式下会先改试其它 MJPEG 分辨率，再改试 YUYV。YUYV 兜底会绑定隐藏预览窗口并请求 RAW 回调，再由 Java 转成 JPEG 用于格子显示、拉流和截图，避免直接预览绿屏和 native NV21 转换卡住。
...
- 日志里出现“格式=RAW”、“YUYV RAW将转JPEG”、“已绑定隐藏预览窗口引出帧回调”或“来源=YUYV->NV21”：说明这一路 YUYV 正在绕开 native NV21 转换，由 Java 尝试把 RAW YUYV 转成 JPEG 给格子显示、拉流和截图使用。
