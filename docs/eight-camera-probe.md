...
- If an opened slot still has no frame callback after 5 seconds, the app automatically reopens that slot up to 2 times. In compatibility recovery mode, retries first try another MJPEG size and then try YUYV. The YUYV fallback binds a hidden preview surface and requests RAW callbacks so Java can convert raw YUYV to JPEG for the on-tile overlay, HTTP streaming, and snapshots while avoiding direct green preview output and native NV21 conversion stalls.
...
- `格式=RAW`, `YUYV RAW将转JPEG`, `已绑定隐藏预览窗口引出帧回调`, or `来源=YUYV->NV21` means YUYV is being driven through a hidden preview surface while bypassing native NV21 conversion, and Java is trying to generate JPEG frames for the on-tile overlay, HTTP streaming, and snapshots.
