...
- If an opened slot still has no frame callback after 5 seconds, the app automatically reopens that slot up to 2 times. In compatibility recovery mode, retries first try another MJPEG size and then try YUYV. The YUYV fallback is diagnostic-only: it proves the format has frames, but does not bind a preview surface or generate JPEG frames, avoiding green preview and crashes.
...
- Compatibility recovery mode is enabled: each camera open prefers 640x480 or lower MJPEG, uses UVCCamera's default 1-30 FPS range, and keeps the bandwidth factor at 1.00. If a route has no frames, the first retry tries another MJPEG size and the second retry tries diagnostic-only YUYV.
...
- `YUYV诊断模式` or `YUYV诊断帧已到达` means YUYV can deliver frames, but the app intentionally skips preview binding and JPEG generation to avoid green preview and crashes.
- `自动重试` means the app detected an opened slot with no frame callback and reopened it. Failure after both MJPEG-size and YUYV retries points more to third-party UVC access or driver compatibility limits for that route; YUYV frames without JPEG indicate the next work is explicit YUYV transcoding.
