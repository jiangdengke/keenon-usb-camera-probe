...
- If an opened slot still has no frame callback after 5 seconds, the app automatically reopens that slot up to 2 times; retries keep MJPEG first and only lower the bandwidth factor when the selected preview is 320x240 or lower.
...
- Strong low-bandwidth diagnosis mode is enabled: each camera open prefers 320x240 or lower MJPEG, first requests 1-10 FPS, and automatically falls back to 1-31 FPS if the camera rejects the low-FPS range. When the selected preview is 320x240 or lower, the bandwidth factor starts at 0.20 and retries with 0.10; when the camera only supports a higher minimum size such as 640x480, the app uses the compatible 1.00 bandwidth factor to avoid high-resolution no-frame starts caused by an overly low factor.
...
- If a camera only advertises higher resolutions, the app uses that camera's smallest supported size, switches to the compatible bandwidth factor, and logs the selected size in Chinese.
...
When one route still has no video, open `显示日志` and check the selected size line for that slot. A line that says the app did not find 320x240 or lower means that camera is still using a higher minimum resolution and the app has switched to the compatible 1.00 bandwidth factor; if that route still has no frame callback, check USB topology, hub power, cable, or the camera itself.
...
- `低FPS不兼容` or `fpsFallback=true` means the camera rejected the 1-10 FPS request, and the app automatically fell back to the wider 1-31 FPS range.
- `自动重试` means the app detected an opened slot with no frame callback and reopened it. Recovery after a low-resolution retry points more to USB bandwidth/scheduling or first-open timing; failure after both retries at 640x480 with compatible bandwidth points more to hardware, hub, cable, or driver limits.
