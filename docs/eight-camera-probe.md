...
- By default the app uses the official Android Camera2/HAL contract from Keenon's `CurrencyCameraActivity`: open camera IDs 0-3 in `cameraIdList` order, set each service-owned SurfaceTexture buffer to exactly 640x480, create a session with that single Surface, and submit an unmodified repeating `TEMPLATE_PREVIEW` request. The old USB/libuvc path remains available by disabling Camera2 mode through ADB extras.
- In Camera2 mode, the foreground service drains each official 640x480 SurfaceTexture through an offscreen EGL readback and generates transport JPEGs without adding a second Camera2 output target. It no longer queries JPEG output sizes or falls back to 1920x1080. The Activity can stop or be covered without destroying Camera2, HTTP MJPEG, or WebSocket resources.
...
- After the app has been opened once and CAMERA permission has been granted, the standard Android `BOOT_COMPLETED` broadcast starts the foreground streaming service without opening the Activity. Tapping `关闭全部` stops only the current run; the next normal device boot restores streaming again.
...
For boot auto-start verification, install and open the app once, grant CAMERA permission, and confirm normal streaming before running `adb reboot`. After `adb wait-for-device`, do not launch the Activity. Check `adb shell dumpsys activity services com.serenegiant.usbcameratest7` and Logcat tags `KeenonBootReceiver` and `KeenonCameraService`; the receiver should log `开机自启已触发`, the foreground notification should appear, and remote WebSocket/HTTP counters should resume increasing. Android versions below 5.0 and devices without CAMERA permission intentionally skip startup. A force-stopped package or robot firmware that blocks third-party boot receivers may require the user to open the app again. Because `BOOT_COMPLETED` is protected, non-root ADB environments may reject attempts to simulate it; use a real reboot as the authoritative check.

For the Camera2 foreground-service path, first confirm that all four slots log `官方SurfaceTexture=640x480`, `request=TEMPLATE_PREVIEW，target=单Surface`, and `官方640x480 Surface首帧JPEG已生成`. The KJPG headers for slot indexes 0-3 must all report `width=640` and `height=480`; no route should report 1920x1080. Then switch to the robot home page or another app for at least 30 seconds. The receiver-side per-slot sequence numbers and the HTTP `/cameras` frame data must continue to increase while the probe Activity is stopped. Returning to the probe must not create a second WebSocket connection or duplicate camera sessions.
...
- `前台推流服务已创建`, `Camera2后台扫描结果`, `官方SurfaceTexture=640x480`, `官方调用已启动`, `官方640x480 Surface首帧JPEG已生成`, or `后台推流服务=运行中` means the foreground service is using the official Camera2 Surface contract and generating transport JPEGs from that Surface. Field users should verify all four KJPG headers remain 640x480, receiver counters continue increasing after switching apps, and `/stream/N.mjpeg` remains reachable.
- `开机自启已触发` means the boot receiver accepted `BOOT_COMPLETED` and requested the existing foreground service to restore streaming. `开机自启已跳过` explains that the Android version or CAMERA permission prerequisite was not met.
